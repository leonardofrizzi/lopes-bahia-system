...
export default function Sidebar() {
  ...
  useEffect(() => {
    const usr = localStorage.getItem('usuarioLogado');
    if (usr) {
      try {
        const parsed = JSON.parse(usr);
        setCargo(parsed.role ?? parsed.cargo ?? null);
      } catch {
        setCargo(null);
      }
    }
  ...
  const menuItems: MenuItem[] = [
    ...commonItems,
    ...(cargo === 'superadmin' ? [configItem] : [])
  ];

  const externalLinks: ExternalLink[] = [
    { name: 'LopesNet Prontos', url: 'https://prontos.lopesnet.com.br/' },
    { name: 'LopesNet Contatos', url: 'https://lopesnet.contact2sale.com/webapp' }
  ];
          ...
          {externalLinks.map(link => (
            <li key={link.name}>
              <a
                ...
                className="flex w-full items-center px-4 py-3 rounded-md text-gray-700 hover:bg-gray-100 transition-colors"
              >
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-[#eb194b] mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
